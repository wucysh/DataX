        ...
        public void prepare() {
            ...
            if(hdfsHelper.isPathexists(path)){
                ...
                if("overwrite".equalsIgnoreCase(writeMode)){
                    LOG.info(String.format("由于您配置了writeMode overwrite, 开始检查 [%s] 下面的内容", path));
                    if(isExistFile){
                        for(Path filePath:existFilePaths){
                            LOG.info(String.format("由于您配置了writeMode overwrite, 已经存在的文件内容有 [%s] ", filePath));
                        }
                        hdfsHelper.deleteFiles(existFilePaths);
                    }
                }
